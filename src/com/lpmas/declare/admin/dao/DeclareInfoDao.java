...
public class DeclareInfoDao {
	...
	public int updateDeclareInfo(DBObject db, DeclareInfoBean bean) {
		int result = -1;
		try {
			String sql = "update declare_info set user_id = ?, declare_type = ?, declare_status = ?, status = ?, modify_time = now(), modify_user = ?, memo = ? where declare_id = ?";
			PreparedStatement ps = db.getPreparedStatement(sql);
			ps.setInt(1, bean.getUserId());
			ps.setInt(2, bean.getDeclareType());
			ps.setString(3, bean.getDeclareStatus());
			ps.setInt(4, bean.getStatus());
			ps.setInt(5, bean.getModifyUser());
			ps.setString(6, bean.getMemo());

			ps.setInt(7, bean.getDeclareId());
